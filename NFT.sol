...
contract LotteryGame is ERC721 {
    ...
    function mint(address to, uint tokenId, string memory content) public virtual {
    ...
    function safeTransferFrom(
        ...
        require(_isApprovedOrOwner(from, tokenId), "ERC721: caller is not token owner nor approved");
        _safeTransfer(from, to, tokenId, data);
    }

    //Function that return the content of a given token
    function getTokenContent(uint tokenId) public view returns(string memory){
    ...
    function setTokenContent(uint tokenId, string memory content) internal {
        require(_exists(tokenId), "ERC721: token does not exist!");
        token_contents[tokenId] = content;
        string memory str = string(abi.encodePacked("Set Content of Token ", Strings.toString(tokenId)));
